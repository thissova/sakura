...
/**
 * Floating WhatsApp shortcut, pinned bottom-right on every public page, with a
 * "Napište nám" label beside the icon. The label is part of the same link, so
 * tapping either opens the chat.
...
export function WhatsAppButton() {
  ...
  return (
    <a
      ...
      // Accessible name keeps the fuller wording and still contains the
      // visible label, as speech-input users need.
      aria-label={t("contact.whatsapp")}
      className="group fixed right-4 md:right-6 z-40 flex items-center gap-3"
      style={{ bottom: "calc(1rem + env(safe-area-inset-bottom, 0px))" }}>
      <span className="whitespace-nowrap rounded-full bg-white px-4 py-2 text-sm md:text-base font-semibold text-sakura-green shadow-lg ring-1 ring-black/5 transition-transform duration-200 group-hover:-translate-x-0.5">
        {t("whatsapp.label")}
      </span>

      <span className="relative flex h-14 w-14 md:h-16 md:w-16 shrink-0 items-center justify-center">
